...
namespace python
{
    ...
    // ReSharper disable once CppMemberFunctionMayBeStatic
    std::string context::as_bytes(const object &value) const // NOLINT(*-convert-member-functions-to-static)
    {
        std::unique_ptr<object> bytes = nullptr;
        if (PyUnicode_Check(rawptr(value))) {
            bytes = std::make_unique<strong_ref>(PyUnicode_AsLatin1String(rawptr(value)));
        } else if (PyBytes_Check(rawptr(value))) {
            bytes = std::make_unique<weak_ref>(rawptr(value));
        } else {
            throw std::logic_error("Unhandled string type");
        }

        auto result = std::string();
        if (const auto size = PyBytes_Size(rawptr(*bytes)); size > 0) {
            const auto string = PyBytes_AsString(rawptr(*bytes));
            if (string == nullptr) {
                throw std::runtime_error("Unable to convert Python bytes to string");
            }
            result.assign(string, size);
        }
        return result;
    }
}
